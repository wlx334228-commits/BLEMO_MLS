classdef SMD6 < PROBLEM 
    methods
        function obj = SMD6()
            ...
            p = obj.Global.D(1) - r;
            q =  floor((obj.Global.D(2) - r)/2 - eps);
            s = ceil((obj.Global.D(2) - r)/2 + eps);
            
            PS = {zeros(1,obj.Global.D(1)),zeros(1,obj.Global.D(2))};
            
            obj.Parameter = table(r,p,q,s,PS);
            
            obj.Global.upper_domain = [repmat(-5,1,p),repmat(-5,1,r);repmat(10,1,p),repmat(10,1,r)];
            obj.Global.lower_domain = [repmat(-5,1,q+s),repmat(-5,1,r);repmat(10,1,q+s),repmat(10,1,r)];
        end
        ...
        function objs = CalObj(obj,Population,type)
            ...
            switch type
                case 'upper'
                    objs = sum((xu1).^2,2) ...
                        - sum(xl1(:,1:q).^2,2) + sum(xl1(:,q+1:q+s).^2,2) ...
                        + sum((xu2).^2,2) - sum((xu2 - xl2).^2,2);
                case 'lower'
                    term2 = sum(xl1(:,1:q).^2,2);
                    for i=q+1:2:q+s-1
        ...
        function P = lower_PF(obj,upper_decs)
            ...
            q = obj.Parameter.('q');
            N = size(upper_decs,1);
            lower_decs = [zeros(N,q),upper_decs(:,p+1:p+r)];
