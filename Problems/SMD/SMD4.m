classdef SMD4 < PROBLEM 
    methods
        function obj = SMD4()
            ...
            obj.Global.upper_domain = [repmat(-5,1,p),repmat(-1,1,r);repmat(10,1,p),ones(1,r)];
            obj.Global.lower_domain = [repmat(-5,1,q),zeros(1,r);repmat(10,1,q),repmat(exp(1),1,r)];
        end
        ...
        function objs = CalObj(obj,Population,type)
            ...
            switch type
                case 'upper'
                    ...
                    objs = sum((xu1).^2,2) ...
                        - sum((xl1).^2,2) ...
                        + sum((xu2).^2,2) - sum((abs(xu2) - log(1+xl2)).^2,2);
                case 'lower'
                    objs(:,1) = sum((xu1).^2,2) ...
        ...
        function P = lower_PF(obj,upper_decs)
            ...
            q = obj.Parameter.('q');
            N = size(upper_decs,1);
            lower_decs = [zeros(N,q),exp(abs(upper_decs(:,p+1:p+r)))-1];
