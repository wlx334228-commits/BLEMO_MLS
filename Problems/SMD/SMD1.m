classdef SMD1 < PROBLEM 
    methods
        function obj = SMD1()
            ...
            obj.Global.upper_domain = [repmat(-5,1,p),repmat(-5,1,r);repmat(5,1,p),repmat(5,1,r)];
            obj.Global.lower_domain = [repmat(-5,1,q),repmat(-pi/2+eps,1,r);repmat(5,1,q),repmat(pi/2-eps,1,r)];
        end
        ...
        function P = lower_PF(obj,upper_decs)
            ...
            q = obj.Parameter.('q');
            N = size(upper_decs,1);
            lower_decs = [zeros(N,q),atan(upper_decs(:,p+1:p+r))];
