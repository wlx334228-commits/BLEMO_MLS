classdef SMD7 < PROBLEM 
    methods
        function obj = SMD7()
            ...
            p = obj.Global.D(1) - r;
            q = obj.Global.D(2) - r;
            PS = {zeros(1,obj.Global.D(1)),[zeros(1,q),ones(1,r)]};
            obj.Parameter = table(r,p,q,PS);
            
            obj.Global.upper_domain = [repmat(-5,1,p),repmat(-5,1,r);repmat(10,1,p),ones(1,r)];
            obj.Global.lower_domain = [repmat(-5,1,q),repmat(0+eps,1,r);repmat(10,1,q),repmat(exp(1),1,r)];
        end
        ...
        function objs = CalObj(obj,Population,type)
            ...
            switch type
                case 'upper'
                    m = repmat(1:p,size(xu,1),1);
                    objs = 1+1/400*sum((xu1).^2,2) - prod(cos(xu1./sqrt(m)),2) ...
                        - sum((xl1).^2,2) ...
        ...
        function P = lower_PF(obj,upper_decs)
            ...
            q = obj.Parameter.('q');
            N = size(upper_decs,1);
            lower_decs = [zeros(N,q),exp(upper_decs(:,p+1:p+r))];
