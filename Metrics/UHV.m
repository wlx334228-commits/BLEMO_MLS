...
%     PopObj(any(PopObj>1,2),:) = [];
    ...
    if ~isempty(PF)
        fmin   = min(min([PopObj;PF],[],'all'),0);
        RefPoint = max(PF-fmin,[],1)*1.1;
    else
        fmin   = min(min(PopObj,[],'all'),0);
        RefPoint = max(PopObj-fmin,[],1)*1.1;
    end

    PopObj = PopObj - fmin;

    PopObj(any(PopObj>repmat(max(PF-fmin,[],1),N,1),2),:) = [];
