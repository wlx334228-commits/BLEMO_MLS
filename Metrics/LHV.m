function [Score,PopObj] = LHV(Pop,Global)
% <metric> <max>
% Hypervolume
...
%--------------------------------------------------------------------------
    Score = [];
    for i=1:length(Pop)
        PF = Global.get_lower_PF(Pop(i).upper_dec);
        PopObj = Pop(i).lower_obj;
        Score = cat(2,Score,HV_dealing(PopObj,PF));
    end
    Score = mean(Score);
end

function Score = HV_dealing(PopObj,PF)
    % Normalize the population according to the reference point set
    [N,M]  = size(PopObj);
    fmin   = min(min(PopObj,[],1),min(PF,[],1));
    fmax   = max(PF,[],1)*1.1;
