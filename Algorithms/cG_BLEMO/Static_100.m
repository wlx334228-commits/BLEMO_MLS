...
%             cgan = cgan0;
%          else
%             cgan = cgan.train(TrainSet.udecs,TrainSet.ldecs,Global.l_D*2); 
%          end

        %% Generate the upper level Offspring
        
        [SQ,RQs] = Reproduce_v20(N,tmax,u_Pop,Idx_ul,l_Pop,Arch_partion,cgan,W,Global);
        
%% Combine parents and offspring, Update the Population
        if ~isempty(SQ)                          
            [u_Pop,Idx_ul,l_Pop,Arch_partion,output,Idx_oA,Archive,RPs,TrainSet] = UL_Select_m2m_v20(l_Pop,output,Archive,RPs,SQ,RQs,W,Global);
