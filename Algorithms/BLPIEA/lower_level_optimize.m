function [Population,History_data] = lower_level_optimize(Population,History_data,T,beta,Maxgen)
    if isempty(Population)
        return;
    end
    
    Global = GLOBAL.GetObj();
    Population = Global.Evaluate(Population,'lower');
    
    N = length(Population);
    
    upper_decs = Population.upper_decs;
    [unique_upper_decs,IA] = uniquetol(upper_decs,0,'ByRows', true,'OutputAllIndices', true);
    if ~isempty(History_data)
        [Dis,Ind] = min(pdist2(unique_upper_decs,History_data.upper_decs),[],2);
    else
        Dis = Inf(1,length(IA));
    end
    
    Archive = cell(length(IA),1);
    lower_RV = Population.lower_objs;
    ArchiveID = 1:N;
    
    for i=1:length(IA)
        ArchiveID(IA{i}) = i;
        Population_i = Population(IA{i});
        if length(IA{i}) > 1
            [~,I] = unique(Population_i.lower_decs,'row');
            Population_i = Population_i(I);
        end
        if isempty(History_data)||Dis(i)>0
            Archive_i = Population_i;
        else
            Archive_i = History_data(Ind(i)).Archive;
            D = min(pdist2(Population_i.lower_decs,Archive_i.lower_decs),[],2);
            Archive_i = cat(2,Archive_i,Population_i(D~=0));
        end
        while length(Archive_i)<Global.N(2)
            lower_decs = mutation(Population_i.lower_decs,1,'lower',{0.1,20});
            Archive_i = cat(2,Archive_i,INDIVIDUAL({unique_upper_decs(i,:),lower_decs}));
            [~,I] = unique(Archive_i.lower_decs,'row');
            Archive_i = Archive_i(I);
        end
        Archive_i = Global.Evaluate(Archive_i,'lower');
        Archive_i = Update_Archive(Archive_i,Global.N(2));
        Archive(i) = {Archive_i};
    end
    
    if ~isempty(History_data)
        History_data(Ind(Dis==0))=[];
    end
    
    t = 0;
    record_index = [];
    indicator = [];
    status = [];
    fitness = [];
    
    rate = ones(N,1);
    while t < Maxgen
        indicator_t = zeros(N,1);
        status_t = zeros(N,1);
        for i = 1:N
            Population_i = Population(i);
            lower_RV_i = lower_RV(i,:);
            if ~ismember(i,record_index) || ...
                    ~Population_i.lower_feasible ||...
                    (ismember(i,record_index) && Population_i.lower_feasible && rand<rate(i))
                Archive_i = Archive{ArchiveID(i)};
                if rand < 0.5 || N < T
                    P = Mating(Population_i,Archive_i,T);
                    lower_dec = DE_current_1_bin(Population_i.lower_dec,P(1).lower_dec,P(2).lower_dec,...
                        'lower',{0.5,0.2,0,20});
                else
                    P = Mating(Population_i,Population,T);
                    lower_dec = DE_current_1_bin(Population_i.lower_dec,P(1).lower_dec,P(2).lower_dec,...
                        'lower',{0.5,0.2,0,20});
                end
                
                Dis = min(pdist2(lower_dec,Archive_i.lower_decs),[],2);
                if Dis>1e-6
                    
                    Offspring_i = Global.Evaluate(INDIVIDUAL({Population_i.upper_dec,lower_dec}),'lower');
                    
                    Dis = min(pdist2(Population_i.lower_objs,Offspring_i.lower_objs),[],2);
                    
                    if Dis==0 && Offspring_i.lower_CV==Population_i.lower_CV
                        
                        Candidate = [Population_i,Offspring_i];
                        Candidate = Global.Evaluate(Candidate,'upper');
                        
                        Archive_i = cat(2,Archive_i,Candidate(2));
                        
                        NDRank = NDSort(Candidate.upper_objs,Candidate.upper_cons,1);
                        if NDRank(2) < NDRank(1)
                            Population_i = Candidate(2);
                            indicator_t(i) = 0;
                        end
                        
                    else
                        Archive_i = cat(2,Archive_i,Offspring_i);
                        Candidate = [Population_i,Offspring_i];
                        
                        if Offspring_i.lower_CV == Population_i.lower_CV
                            [Population_i,pos] = Tchebycheff_Select(Candidate,Archive_i,lower_RV_i);
                            if pos==2
                                indicator_t(i) = 1;
                            else
                                indicator_t(i) = 0;
                            end
                        else
                            [~,pos] = min(Candidate.lower_CVs);
                            Population_i = Candidate(pos);
                            if pos==2
                                indicator_t(i) = 1;
                            end
                        end
                    end
                    
                    Archive_i = Update_Archive(Archive_i,Global.N(2));
                    
                    Archive(ArchiveID(i)) = {Archive_i};
                    Population(i) = Population_i;
                    status_t(i) = 1;
                end
            end
        end
        
        indicator = cat(2,indicator,indicator_t);
        status = cat(2,status,status_t);
        
        fitness = cat(2,fitness,sum(indicator_t==1)/(sum(status_t)+eps));
        
        rate = 1-sum(indicator==1,2)./sum(status,2);
        
        if  t>=beta
            record_index = find(all(indicator(:,t-beta+1:end)==0,2));
            if all(fitness(t-beta+1:end)<0.1)
                break;
            end
        end
        
%         Global.lower_Output(Population);
        
        t=t+1;
    end
    
    Current_data = Data_dealing(unique_upper_decs,Archive);
    Population = Update(Population,Current_data,IA,lower_RV);
    Global.lower_Output(Population);
    
    History_data = cat(2,History_data,Current_data);
end

function P = Mating(Individual,Data,T)
    lower_obj = Individual.lower_obj;
    Findex = find(Data.lower_feasibles);
    if length(Findex)>T
        B = pdist2(lower_obj,Data(Findex).lower_objs);
        B(B==0) = inf;
        [~,B] = sort(B,2);
        B = B(:,1:T);
        if rand < 0.5
            P = B(randperm(T,2));
        else
            P = randperm(length(Findex),2);
        end
        P = Findex(P);
    else
        FrontNo = NDSort(Data.lower_objs,Data.lower_CVs,inf);
        P=ones(1,2);
        while P(1)==P(2)
            P = TournamentSelection(2,2,FrontNo);
        end
    end
    P = Data(P);
end

function [Population,index] = Tchebycheff_Select(Candidate,Archive,RV)
    lower_Zmin = min([RV;Archive.lower_objs],[],1);
    RV = RV-lower_Zmin;
    RV = RV./sum(RV,2);
    index = false(1,length(Candidate));
    for i=1:size(RV,1)
        inds = find(~index);
        [~,pos] = min(max((Candidate(inds).lower_objs-lower_Zmin)./RV(i,:),[],2));
        index(inds(pos)) = true;
    end
    index = find(index);
    Population = Candidate(index);
end

function Archive = Update_Archive(Archive,N)
    if length(Archive)> N
        LFS = find(Archive.lower_feasibles);
        if length(LFS)> N
            Archive = NSGAII_Update(Archive(LFS),N,'lower');
        else
            [~,rank] = sort(Archive.lower_CVs,'ascend');
            Archive = Archive(rank(1:N));
        end
    end
end

function Data = Data_dealing(upper_decs,Archive)
    Global = GLOBAL.GetObj();
    S = length(Archive);
    Best_Archive = cell(S,1);
    for i = 1:S
        Data_i = Archive{i};
        [~,index] = Data_i.lower_best;
        
        Data_i(index) = Global.Evaluate(Data_i(index),'upper');
        inds = find(Data_i(index).upper_feasibles);
        if ~isempty(inds)
            [~,best] = Data_i(index(inds)).upper_best;
            Best_Archive_i = Data_i(index(inds(best)));
        else
            [~,best] = min(Data_i(index).upper_CVs);
            Best_Archive_i = Data_i(index(best));
        end
        
        for j=1:length(Best_Archive_i)
            Best_Archive_i(j).label = true;
        end
        Archive{i} = Data_i;
        Best_Archive{i} = Best_Archive_i;
    end
    
    Data = DATA(upper_decs,Archive,Best_Archive);
end

function Population = Update(Population,Data,IA,lower_RV)
    S = length(Data);
    for i = 1:S
        Best_Archive_i = Data(i).Best_Archive;
        L = length(Best_Archive_i);
        ID = IA{i};
        K = length(ID);
        if L>K
            lower_objs = Best_Archive_i.lower_objs;
            
            lower_RV_i = lower_RV(ID,:);
            lower_Zmin = min([lower_RV_i;lower_objs],[],1);
            lower_Zmax = max([lower_RV_i;lower_objs],[],1);
            
            lower_RV_i = max((lower_RV_i-lower_Zmin)./(lower_Zmax-lower_Zmin),1e-6);
            lower_RV_i = max(lower_RV_i./sum(lower_RV_i,2),1e-6);
            
            Next = false(1,L);
            for j=1:K
                index = find(~Next);
                [~,pos] = min(max((lower_objs(index,:)-lower_Zmin)./(lower_Zmax-lower_Zmin)./lower_RV_i(j,:),[],2));
                Next(index(pos)) = true;
            end
            Population(ID) = Best_Archive_i(Next);
        else
            Population(ID) = [Best_Archive_i,Best_Archive_i(randi(L,1,K-L))];
        end
    end
    
end
