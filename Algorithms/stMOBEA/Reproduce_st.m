...
%% loading data in l_Pop
    ...
    l_xl = zeros(Nu,Global.D(2));
    for i = 1:Nu
        l_xu(i,:) = l_Pop{i}(1).upper_dec;
        l_xl(i,:) = l_Pop{i}(1).lower_dec;
    end
...
%% Select the elite UL population from SQ for LL search    
    ...
    uPa = l_xu;
    lPa = l_xl;
    pcon = [u_Pop.upper_cons,u_Pop.lower_cons,zeros(length(pop),1)];
...
%         lQu = reshape(lQu,size(lQu,1),l_D);
        ...
        uQu = DE_current_1_bin(uP1,uP2,uP3,'upper');
        lQu = DE_current_1_bin(lP1,lP2,lP3,'lower');
        
        if tmax>1
            off = Global.Evaluate(INDIVIDUAL({uQu,lQu}));
            offcon = [off.upper_cons,off.lower_cons,planemodel.LL_discriminator(off)];
            R = [pop,off];
            Rcon = [pcon;offcon];
...
%         pause(0.00001);      
        ...
        if ~isempty(uQu)
            ...
            for i = 1:size(uQu,1)
                ...
                if tmax>1
                    site = ismember(r.upper_decs,Au(I,:),'rows');
                    [SQ{I-nt},RQs{I-nt}] = LLsearch_st(r(site).upper_decs,Pt,Global);                   
                else
                    [SQ{I-nt},RQs{I-nt}] = LLsearch_st(Au(I,:).upper_decs,Pt,Global); 
                end
                Archive{I} = SQ{I-nt};
            end
        else
...
function [pop,pcon,Idx_sp,Pa,Arch_partion]= m2mSelection(R,Rcon,W,Z,Nu,K,S)
     ...
     if sum(site)<floor(Nu*1.5)
        [FNo,~] = NDSort(R.upper_objs,Rcon,inf); 
    ...
    [FrontNo,MaxFNo] = NDSort(R.upper_objs,Rcon,Nu);
