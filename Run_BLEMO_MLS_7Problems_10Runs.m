...
% and running time.
    ...
    problems = {
        'TP1',   [1 2],   [10 10  60], 5e5; ...
        'TP2',   [1 14],  [10 30 300], 5e5; ...
        'DS1_1', [10 10], [40 10 400], 1.5e6; ...
        'DS2_1', [10 10], [40 10 400], 3e6; ...
        'DS3_1', [10 10], [40 10 400], 3e6; ...
        'DS4',   [1 9],   [20 40 200], 1.5e6; ...
        'DS5',   [1 9],   [20 40 200], 1.5e6; ...
    };
    ...
    for p = 1:size(problems,1)
        ...
        for run = 1:runs
            ...
            fprintf('Running %s on %s, run %d/%d, maxFE=%g\n', ...
                algorithm, problemName, run, runs, maxFE);
...
function initializeSummaryFiles(csvFile,matFile,errorLogFile,rows,problems,metrics)
    ...
    fprintf(fid,'Problem,Run,LGD,HV,UIGD,FEsu,FEl,RunningTime,MaxFE,Status,ErrorID,ErrorMessage,MatFile\n');
    clear cleaner;
...
function appendRowToCsv(row,csvFile)
    ...
    assert(fid>0,'RunScript:FileOpenError','Cannot open summary CSV: %s',csvFile);
    cleaner = onCleanup(@()fclose(fid));
    fprintf(fid,'%s,%d,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%.16g,%s,%s,%s,%s\n', ...
        csvText(row.Problem), ...
        row.Run, ...
        row.LGD, ...
        row.HV, ...
...
function Summary = rowsToTable(rows)
    variableNames = {'Problem','Run','LGD','HV','UIGD','FEsu','FEl', ...
        'RunningTime','MaxFE','Status','ErrorID','ErrorMessage','MatFile'};
    if isempty(rows)
